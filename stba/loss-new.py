...
def compute_total_loss(logits, label, flow, lambda_, targeted=False):
    """
    Compute the total loss for STBA:
        L_total = L_adv + lambda * L_f

    Args:
        logits (Tensor): Model output logits, shape [B, num_classes]
        label (Tensor): Ground-truth or target label, shape [B]
        flow (Tensor): Flow field, shape [B, 2, H, W]
        lambda_ (float): Weight for flow smoothness loss (from config)
        targeted (bool): If True, compute targeted attack loss

    Returns:
        total_loss (Tensor): Combined total loss
        loss_adv (Tensor): Adversarial loss (for logging)
        loss_flow (Tensor): Flow smoothness loss (for logging)
    """

    # === Equation (6) and (8): Adversarial loss ===
    if targeted:
        # Equation (8): minimize CE for the target class
        loss_adv = F.cross_entropy(logits, label)
    else:
        # Equation (6): maximize CE (i.e., minimize correct class)
        loss_adv = -F.cross_entropy(logits, label)

    
    # === Equation (7) and (9): Flow smoothness loss ===
    # Compute finite differences along x and y axes
    flow_x_diff = flow[:, :, :, 1:] - flow[:, :, :, :-1]  # Horizontal gradient
    flow_y_diff = flow[:, :, 1:, :] - flow[:, :, :-1, :]  # Vertical gradient

    # Mean TV norm as stated in paper (Eq. 9)
    loss_flow = flow_x_diff.abs().mean() + flow_y_diff.abs().mean()

    # === Total loss ===
    # Final objective: L = L_adv + lambda * L_f
    total_loss = loss_adv + lambda_ * loss_flow
    #print(f"[step loss_adv: {loss_adv:.4f}, loss_flow: {loss_flow:.4f}, lambda: {lambda_}, total_loss: {total_loss:.4f}]")
    return total_loss, loss_adv.detach(), loss_flow.detach()
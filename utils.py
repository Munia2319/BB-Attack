...
def flow_st(images, flows):
    ...
    # Create a base coordinate grid representing the original pixel positions for each image in the batch.
    # This grid holds the (y, x) coordinates for every pixel before any transformation is applied.
    # We will later add a learned flow (shift) to this grid to produce a new sampling grid.
    # This allows each pixel in the output image to be constructed by bilinearly interpolating from 
    # a shifted position in the input image — enabling smooth spatial transformations like warping.
    ...
    #To apply bilinear interpolation, pixel values must be last, not first.
    #Reorder image dims to use pixel-wise interpolation
...
def set_seed(seed=42):
    ...
    Making sure that results are consistent(not different) across runs.
    """
    random.seed(seed)
    ...
    # If True, CuDNN tries to find the best algorithm for hardware → can lead to non-determinism.
    torch.backends.cudnn.benchmark = False
    #Guarantees consistent results across runs — at the cost of slightly slower training or inference.
...
def seed_data_and_model(args):
    ...
    if args.dataset == "CIFAR-10":
        args.image_size = 32
        # Set image dimensions for CIFAR-10 , 3= number of channels, 32= height and width, can be used for creating dummy input image
        ...
        if args.target_model == "defense":
            ...
            #args.model_names = ["Wong2020Fast","Sehwag2020Hydra", "Wang2020Improving", "Zhang2019Theoretically"]
            args.model_names = ["Wong2020Fast"]

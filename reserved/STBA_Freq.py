    ...
    def lffa_attack(self, args, x_high, x_low, y, t=None):
        ...
        with torch.no_grad():
            ...
            for i in tqdm(range(num_iters), disable=True):
                # Check attack success (optional enhancement)
                for dx, dy in args.shift_set:
                ...
                # Step 4: average loss over all (dx, dy)
...
def attack(args):
    ...
    for model_name in args.model_names:
        ...
        # Load the model which is either clean or a defense model and also normalize the images before feeding them
        model = load_clean_model(args)
        # Load the model to the specified device
        model = model.eval().to(device)
        # applying main attack strategy
...
if __name__ == "__main__":
    ...
    parser.add_argument('--shift_set', type=str, default="[(0,0), (2,0), (-2,0), (0,2), (0,-2)]",
                    help='Translation shifts as list of tuples, e.g., "[(0,0), (1,0), (-1,0)]"')
    

    args = parser.parse_args()
    #convert this string to an actual list
